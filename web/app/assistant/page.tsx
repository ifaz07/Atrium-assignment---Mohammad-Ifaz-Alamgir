...
export default function AssistantPage() {
  ...
  return <main><section className="page-heading"><div><p className="eyebrow">One assistant</p><h1>Atrium assistant</h1><p>Your permissions come from your current session. Do not share passwords or payment details here.</p></div></section><section className="chat-panel" aria-label="Atrium assistant conversation">{entries.map((entry, index) => <article className={entry.from === 'You' ? 'chat-message chat-user' : 'chat-message'} key={index}><strong>{entry.from}</strong><p>{entry.text}</p></article>)}{sending ? <p className="muted">Thinking…</p> : null}</section><form onSubmit={send} className="chat-form"><label><span>Message</span><input value={message} onChange={event => setMessage(event.target.value)} maxLength={2000} placeholder="For example: What sessions are available?" /></label><button disabled={sending} type="submit">Send</button></form></main>;
}
